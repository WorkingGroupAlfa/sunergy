...
export async function PUT(request: Request) {
  ...
  try {
    ...
    console.error(error);
    return NextResponse.json({ error: 'Unable to save about content' }, { status: 500, headers: noStoreHeaders });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error(error);
    return NextResponse.json({ error: 'Unable to reset about content' }, { status: 500, headers: noStoreHeaders });
  }
}
