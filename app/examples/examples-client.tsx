...
export function ExamplesClientPage() {
  const { cases } = useShop();

  return (
    <main className="section-shell py-12">
      <span className="badge">Наші роботи</span>
      ...
      <div className="mt-8 space-y-6">
        {cases.map((item) => (
          <article key={item.slug} className="panel-card grid overflow-hidden md:grid-cols-[1fr_1.1fr]">
            <Image src={item.image} alt={item.title} width={920} height={600} unoptimized={item.image.startsWith('data:image/')} className="h-full w-full object-cover" />
